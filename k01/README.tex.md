...
l.8:説明より平均を求め、結果を返す関数を定義

l.11:説明より分散を求め、結果を返す関数を定義
...
l.31: fnameで指定したファイルの先頭アドレスをファイルポインタ fp に読み込む

l.32: もしfpがNULLだったときは，ファイルを開けなかったので，即座にプログラム終了

l.37: ファイルを1行ずつbufに読み込む．もし読み込み成功する限り以下を繰り返す

l.38: bufからscanfで数値文字列を実数としてvalに読み込む

l.39: 繰り返しの回数をカウントアップする

l.41: 入力された値から平均を計算

l.42: 入力された値から分散を計算

l.43: 入力された値から2乗平均を計算
